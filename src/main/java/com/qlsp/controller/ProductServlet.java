...
import com.qlsp.service.ProductServiceImpl;
...
public class ProductServlet extends HttpServlet {
    ...
    private void createProduct(HttpServletRequest request, HttpServletResponse response) {
        ...
        System.out.println("request"+request.getParameter("unitPrice"));

        if(request.getParameter("unitPrice").isEmpty()){
         request.setAttribute("message", "Khong duoc de trong");
         RequestDispatcher dispatcher = request.getRequestDispatcher("product/create.jsp");
         try {
             dispatcher.forward(request, response);
         } catch (ServletException | IOException e) {
        ...
        if(request.getParameter("quantityStock").isEmpty()){
            request.setAttribute("message", "Khong duoc de trong");
            RequestDispatcher dispatcher = request.getRequestDispatcher("product/create.jsp");
            try {
                dispatcher.forward(request, response);
        ...
        Product product = new Product(productId, name, unitPrice, quantityStock, productDescription, thumbnail, categoryId);
    ...
    private void viewProductByName(HttpServletRequest request, HttpServletResponse response) {
        ...
        for (int i = 0; i < this.productService.findAll().size(); i++) {
            if (this.productService.findAll().get(i).getName().equals(name)) {
                product = this.productService.findAll().get(i);
            }
        }
        RequestDispatcher dispatcher;
